<div id="wrap">
  <!-- footer -->
  <div id="footer">
    <div class="container-footer">
      <div class="partner" data-jarallax class="jarallax">
        <img class="jarallax-img" src="assets/img/Architecture-Partner-Background.jpg" alt="">
        <p class="menutitle">OUR PARTNERS & CLIENTS</p>
        <div class="partner-member">
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
          <div><img src="assets/img/Architecture-Partner-01.jpg" alt=""></div>
        </div>
      </div>
      <div class="overview">
        <p class="menutitle">contact us</p>
        <h2 class="title">Feel free to contact us</h2>
        <div class="contact">
          <div class="address">
            <div class="logo">
              <img src="assets/img/icon/Architecture-option22-location.svg" alt="">
            </div>
            <p># 2188, St. 156, Toeuk Laork II,</p>
            <p>Toul Kork, Phnom Penh, Cambodia.</p>
          </div>
          <div class="phone">
            <div class="logo">
              <img src="assets/img/icon/Architecture-option22-phone.svg" alt="">
            </div>
            <p>+855 23 424 165</p>
            <p>+855 89 888 168 / +855 77 777 678</p>
          </div>
          <div class="email">
            <div class="logo">
              <img src="assets/img/icon/Architecture-option22-Email.svg" alt="">
            </div>
            <p><a href="mailto:info@archsite.com">
                info@archsite.com
              </a></p>
            <p><a href="https://www.facebook.com/archsite">
                www.facebook.com/archsite
              </a></p>
          </div>

        </div>
      </div>
      <div class="mediafooter">
        <ul class="ul-media">
          <li><a href="#" title=""><img src="assets/img/icon/facebook.svg" alt=""></a></li>
          <li><a href="#" title=""><img src="assets/img/icon/youtube.svg" alt=""></a></li>
        </ul>
      </div>
      <div class="footer-bottom">
        <p>© 2018. All rights reserved. ARCHSITE, Sitemap</p>
        <p>Developed by: TomnerbTECH</p>
      </div>
    </div>
  </div>
</div><!-- end-footer -->
</body>
</html>
<link rel="stylesheet" type="text/css" href="assets/css/responsive.css">
<script type="text/javascript" src="assets/js/bootstrap.min.js"></script>
<script type="text/javascript" src="assets/js/carouselswipe.js"></script>
<script type="text/javascript" src="assets/js/main.js"></script>
<script type="text/javascript" src="assets/vender/jarallax.js"></script>
<script type="text/javascript" src="assets/vender/jarallax-element.min.js"></script>
<script type="text/javascript" src="assets/vender/slick-slider/slick.min.js"></script>
<script type="text/javascript" src="assets/vender/wow.min.js"></script>
<script>
    new WOW().init();
  </script>  
<script type="text/javascript" src="assets/vender/myPluginStyle.js"></script>